-- MyMG intern take-home: seed data
--
-- Save this as supabase/seed.sql. Supabase runs it after the migrations on
-- `supabase start` (first time) and on every `supabase db reset`.
--
-- Booking and time-off dates are generated from the moment this runs, so a
-- reset always gives you future dates. Look at the tables after it runs.
--
-- You can add your own rows below if you like, but don't change or remove
